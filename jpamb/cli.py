...
def build(suite, compile, decompile, document, test, docker):
    ...
    if compile:
        ...
        run(
            ...
            + [str(a.relative_to(suite.workfolder)).replace("\\", "/") for a in suite.sourcefiles()],
            logerr=log.warning,
            logout=log.info,
            timeout=600,
        )
        ...
        for cl in suite.classes():
            ...
            res, t = run(
                ...
                + [
                    "jvm2json",
                    "-s",
                    str(suite.classfile(cl).relative_to(suite.workfolder)).replace("\\", "/"),
                ],
                logerr=log.warning,
            )
            ...
            try:
                res, x = run(
                    ...
                    + [
                        "java",
                        "-cp",
                        str(folder.relative_to(suite.workfolder)).replace("\\", "/"),
                        "-ea",
                        "jpamb.Runtime",
